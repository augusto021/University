...
/*   Módulo C -- Compressão do ficheiro original.
      * Daniel Constantino Martins Faria - a93187
      * João Augusto Macedo Moreira- a93326

Este módulo é chamado, recebendo como input, o nome do ficheiro, ao qual vai ser acrescentado ".cod" para
poder receber os códigos correspondentes a cada caracté§r.

As duas funções principais deste módulo são:

    ** matrix_code

  - Esta função é responsável pela criação de uma matriz que irá albergar todos os códigos presentes
  no ficheiro ".cod" para facilitar a procura dos mesmos.
  - Começamos por receber um buffer que contém toda a informação presente no ficheiro ".cod", de
  seguida iremos ler esse mesmo buffer pouco a pouco, carregando a informação na struct FICHEIROCOD
  para posterior utilização.
  - A matriz é carregada, para cada bloco, com strings correspondentes ao código de cada caracter, por ordem
  de forma a que o indice na linha, destas seja igual ao valor inteiro correspondente ao caracter que estas
  codificam.
  - Ao correr o buffer também é lido o modo do ficheiro, o número de blocos, o tamanho de cada bloco, e o
  tamanho máximo entre todos os códigos.

}

    ** printfile

  - Esta função foi criada com o intuito da impressão do ficheiro final, isto é o ".shaf".
  - Começamos por receber o nome do ficheiro de modo um ficheiro com o mesmo nome, mas com a adição da
  extensão ".shaf"
  - Após a criação do ficheiro começamos a codificação de cada bloco do ficheiro original na sequência
  de 0's e 1's, isto é atingido através da função auxiliar: new_string. Esta função retorna uma string com a
  sequência pronta a ser "imprimida".
  - Para imprimir esta nova string utilizamos a função auxiliar char_to_print que pega no tamanho da sequência
  (que será sempre 8 exceto para o último caracter), e irá atribuir um caracter ASCII a que corresponde essa
  sequência.
  - Após a obtenção do caracter este é imprimido no ficheiro ".shaf", e este processo é repetido para todos os
  blocos.
