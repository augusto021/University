/**
@file Interface.c
Ficheiro com as funções da interface.
*/
#include <stdio.h>
#include <string.h>
#include <time.h>
...
/** Função que imprime o tabuleiro de forma personalizada.*/
void mostar_tabuleiro_elegante(ESTADO *e){
    int i, j, linha, letras;
    ...
    if (fp == NULL) {
        ...
        while(fgets(buffer, BUF_SIZE, fp) != NULL) {
            if (l<8) {
                ...
                int num_tokens = sscanf(buffer, "%d: %s %s", &num_jog, jog1, jog2);
                if (num_tokens == 3) {
...
void pos_nao_atualiza(ESTADO *e, int n_jogada){
    ...
    mostar_tabuleiro_elegante(novo);
    prompt(novo);
